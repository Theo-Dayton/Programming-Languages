...
fun freeIn (VAR x) y = if x = y then true else false
  | freeIn (LITERAL v) y = false
  | freeIn (SET (x, z)) y = if y = x then true else freeIn z y
  | freeIn (APPLY (x,xs)) y = List.exists (fn (xs) => freeIn xs y) (x::xs)
  | freeIn (IFX (x,z,w)) y = List.exists (fn (a) => freeIn a y) [x,z,w]
  | freeIn (WHILEX (x,z)) y = List.exists (fn (a) => freeIn a y) [x,z]
  | freeIn (BEGIN xs) y = List.exists (fn (x) => freeIn x y) xs
  | freeIn (LAMBDA (xs, x)) y = freeIn x y andalso (not (member y xs))
  | freeIn (LETX (l, [], e)) y = freeIn e y 
  | freeIn (LETX (LET, zs, e)) y = List.exists (fn (x) => freeIn (snd x) y)
                                  zs orelse (not (member y (List.map fst zs))
                                    andalso freeIn e y)
  | freeIn (LETX (LETSTAR, z::zs, e)) y = freeIn (LETX (LET, [z], LETX
                                          (LETSTAR, zs, e))) y
  | freeIn (LETX (LETREC, zs, e)) y = (List.exists (fn (x) => freeIn (snd x)
                                    y) zs orelse freeIn e y) andalso not
                                    (member y (List.map fst zs))



      local val x_to_5 = SET("x", LITERAL (NUM 5))

      in

      val () = Unit.checkAssert "freeIn var"
      (fn () => not (freeIn (VAR "a") "b"))

      val () = Unit.checkAssert "freeIn literal"
      (fn () => freeIn x_to_5 "x")

      val () = Unit.checkAssert "equal free"
      (fn () => freeIn (LAMBDA (["x"], LAMBDA (["y"], APPLY (VAR "equal?",[VAR
            "x", VAR "y"])))) "equal?")

      val () = Unit.checkAssert "equal x free"
      (fn () => freeIn (LAMBDA (["y"], APPLY (VAR "equal?", [VAR "X",
                    VAR "y"]))) "equal?")

      end

fun improve ((xs, exp), env) = ((xs, exp), List.filter (fn(a) => freeIn
                              exp (fst a)) env)
